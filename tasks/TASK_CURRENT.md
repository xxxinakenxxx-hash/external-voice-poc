...
最終更新：2026-07-16（EX-09のNodeモックDOMによるフロントエンド動作確認を実施し、GPS成功・拒否時のJavaScript分岐、近隣候補表示、店舗選択、手入力時の選択解除、部分一致名寄せ、submit payloadの組み立て、二重送信防止、unsent_textの成功時削除・失敗時保持、トークンなし直接アクセス時の送信無効化を確認した。`src/05_token.gs` の submit 分岐に `latitude`・`longitude`・`storeName` を渡す修正を行い、`clasp push --force` まで完了した。）

---

## 1. 現在地

- 現在の状態：**EX-09の静的構築・公開反映・NodeモックDOM確認は完了。実機確認では送信成功したがS・T・U列が空欄だったため、`src/05_token.gs` の `doPost()` で `submitMemo()` へ `latitude`・`longitude`・`storeName` を渡す修正を行った。`clasp push --force` は完了。送信API用デプロイBの再デプロイ待ち。実通信・実シート保存・S〜U列確認・iPhone Safari実機確認は未実施。過去の実機確認依頼とChrome関連操作・確認結果は無効。EX-09は停止ではなく、実機確認方法の立て直し待ち。EX-09完了判定前。後続工程未着手。** `external/input-page/index.html` のGPS取得・候補表示・手入力解除・名寄せ・送信1回の挙動をNodeモックDOMで確認済み。`src/` 配下と GAS 側関数は未変更。
