...
最終更新：2026-07-15（EX-08の実装・静的チェック・`clasp push --force`・Git pushまで完了。`src/05_token.gs` に API-14 `getStoreCandidates` と API-15 `searchStoresByName` を新規実装し、現行の `doPost(e)` に action 振り分けを追加した。`external/voice-poc/index.html` に action 選択・latitude・longitude・keyword を追加し、`external/voice-poc/README.md` と `tasks/TASK_CURRENT.md` を更新した。GitHub Pages は最終的に反映済み、EX-08実機確認待ち、EX-09未着手、完了判定未実施。）

---

## 1. 現在地

- 現在の状態：**EX-08の実装・静的チェック・`clasp push --force`・Git push完了、GitHub Pages反映済み、EX-08実機確認待ち、EX-09未着手。** `doPost(e)` の現行実体は `src/05_token.gs` のままで、`doPostMemo(e)` は存在しないため、その前提に合わせて action 振り分けを入れた。GPS取得方式の案イや EX-09 の分離方針はそのまま維持し、EX-08では店舗検索の通信疎通だけを実装した。公開ページには action 選択欄が反映された。
- 次にやること：**EX-08実機確認待ち**
