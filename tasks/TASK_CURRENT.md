...
最終更新：2026-07-14（EX-05「署名付きトークンの発行・検証」は実装完了後の静的確認で要修正2点が見つかり、`src/05_token.gs` の `verifyToken` を修正した。payload は `uh`・`exp` の2キーのみを許可し、`active_flag` は `有効` の完全一致だけを許可するように改めた。`src/01_auth.gs` は TOP 画面返却とトークン発行の入口に改訂し、`src/05_token.gs` を新規追加して `issueToken`・`verifyToken`・`doPost`・`getTokenSecret`・`INPUT_PAGE_URL` 参照を実装した。`src/top.html` を新規作成し、TOP 画面から外部入力ページへ `#token=` 付きリンクを表示するようにした。`doPost` はトークン検証と担当者メールアドレスの確定までで止め、AI解析・スプレッドシート書き込み・`submitMemo` 接続はしていない。`clasp push --force` 成功、Git commit `217c567` / push `origin/master` 成功。`TOKEN_SECRET`・`INPUT_PAGE_URL` のスクリプトプロパティ設定、デプロイA・Bの作り直し、正しいトークン・改ざんトークン・トークンなしの実機確認はユーザー確認待ち。EX-06へは進まない。

---

## 1. 現在地

- 現在の状態：**EX-05 署名付きトークンの発行・検証は要修正2点の静的修正を完了し、ユーザー実機確認待ち。** `src/05_token.gs` の `verifyToken` を修正し、payload を `uh`・`exp` の2キーだけに制限し、`active_flag` は `有効` の完全一致のみを許可するようにした。`src/01_auth.gs` を TOP 画面返却とトークン発行の入口に改訂し、`src/05_token.gs` を新規追加して `issueToken`・`verifyToken`・`doPost`・`getTokenSecret`・`INPUT_PAGE_URL` 参照を実装した。`src/top.html` を新規作成し、TOP 画面から外部入力ページへ `#token=` 付きリンクを表示するようにした。`doPost` はトークン検証と担当者メールアドレスの確定までで止め、AI解析・スプレッドシート書き込み・`submitMemo` 接続はしていない。`src/appsscript.json` は今回変更していない。`clasp push --force` 成功、Git commit `217c567` / push `origin/master` 成功。`TOKEN_SECRET`・`INPUT_PAGE_URL` のスクリプトプロパティ設定、デプロイA・Bの作り直し、正しいトークン・改ざんトークン・トークンなしの実機確認はユーザー確認待ち。EX-06以降には進まない。
...
- Git操作結果：これから `src/05_token.gs` と `tasks/TASK_CURRENT.md` をコミットし、既存運用どおり push する。
