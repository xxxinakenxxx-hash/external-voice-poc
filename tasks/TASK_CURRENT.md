...
最終更新：2026-07-16（EX-08検証環境の修正を完了。`external/voice-poc/index.html` に GAS送信先URL欄とトークン欄を画面上部へ戻し、submit の送信本文に latitude／longitude／storeName を含めるようにした。`src/05_token.gs` に検証専用の `issueShortLivedTokenForTest()` を再追加した。`external/voice-poc/README.md` と `tasks/TASK_CURRENT.md` を更新した。静的チェック、`clasp push --force`、Git commit/push は完了。GitHub Pages は確認時点で旧内容のままで、更新待ち。EX-08実機確認待ち、EX-09未着手、完了判定未実施。）

---

## 1. 現在地

- 現在の状態：**EX-08検証環境の修正完了、静的チェック・`clasp push --force`・Git commit/push完了、GitHub Pages更新待ち、EX-08実機確認待ち、EX-09未着手。** `doPost(e)` の現行実体は `src/05_token.gs` のままで、`doPostMemo(e)` は存在しないため、その前提に合わせて action 振り分けを入れた。GPS取得方式の案イや EX-09 の分離方針はそのまま維持し、EX-08では店舗検索の通信疎通だけを実装した。公開ページには action 選択欄が反映された。
- 次にやること：**EX-08実機確認待ち**
...
- 変更したファイル：`external/voice-poc/index.html`、`external/voice-poc/README.md`、`src/05_token.gs`、`tasks/TASK_CURRENT.md`
- 変更していない内容：`external/input-page/` 配下、`getStoreCandidates` 本体、`searchStoresByName` 本体、`verifyToken`、`submitMemo`、`callAiApi`、`parseAiResponse`、`writeRecordsToSheet`、`logError`、正本文書、EX-09実装
- 静的チェック結果：`src/05_token.gs` を一時 `.js` にして `node --check` 成功。`external/voice-poc/index.html` の `<script>` 部分を抽出して `node --check` 成功。いずれも一時ファイル削除済み。
- `clasp push --force` の実行結果：成功。`src` 配下 9 ファイルを反映した。
- Git操作結果：commit `8e71ffb` を作成し、`origin/master` と `origin/main` へ push 成功。
- GitHub Pages反映結果：確認時点では `GAS接続設定` がまだ見えず、更新待ち。デプロイB更新は行わない。
- 次の状態：EX-08実機確認待ち。EX-09未着手。EX-08完了判定未実施。
