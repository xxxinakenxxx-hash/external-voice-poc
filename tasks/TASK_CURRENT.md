...
最終更新：2026-07-14（EX-05R「トークン検証の確認手段の整備」を実装・反映済み。`external/voice-poc/index.html` を検証専用ページとして改修し、GAS送信先URL、トークン、inputText の入力欄、`#token=` 自動反映、HTTPステータスと応答本文の表示、`text/plain;charset=UTF-8` の POST を追加した。`src/05_token.gs` に検証専用の短命トークン発行関数 `issueShortLivedTokenForTest` を追加し、EX-05の検証ロジック本体は変更していない。旧関数名 `issueShortLivedTokenForTest_` は末尾アンダースコア付きのため GAS エディタの関数選択欄に表示されず、手動実行できなかったため、新関数名へ修正した。`external/voice-poc/README.md` を検証専用ページの説明に更新した。`external/input-page/` は変更していない。`clasp push --force`、Git push、GitHub Pages反映確認まで完了。EX-05は継続中で、正しいトークン・1文字改ざん・空欄は実機確認済み、期限切れトークンは未確認。EX-06へは進まない。

---

## 1. 現在地

- 現在の状態：**EX-05R 実装済み、GitHub Pages反映確認済み、EX-05継続中、けんたろうさんの実機確認待ち。** `external/voice-poc/index.html` を検証専用ページとして改修し、GAS送信先URL、トークン、inputText の入力欄、`#token=` 自動反映、HTTPステータスと応答本文の表示、`text/plain;charset=UTF-8` の POST を追加した。`src/05_token.gs` に検証専用の短命トークン発行関数 `issueShortLivedTokenForTest` を追加し、EX-05の検証ロジック本体は変更していない。旧関数名 `issueShortLivedTokenForTest_` は末尾アンダースコア付きのため GAS エディタの関数選択欄に表示されず、手動実行できなかったため、新関数名へ修正した。`external/voice-poc/README.md` を検証専用ページの説明に更新した。`external/input-page/` は変更していない。`clasp push --force`、Git push `origin/master` / `origin/main`、GitHub Pages反映確認まで完了。EX-05は継続中で、正しいトークン・1文字改ざん・空欄は実機確認済み、期限切れトークンは未確認。EX-06へは進まない。
