...
最終更新：2026-07-17（OUT-03を実装し、`src/summary.html` を追加して `clasp push --force` と Git push を完了した。デプロイAの更新は未実施。OUT-03の公開確認は未実施。）

---

## 1. 現在地

- 現在の状態：**OUT-03実装・静的チェック・`clasp push --force`・Git push 完了。`?view=summary` を追加し、`src/summary.html` を新規作成した。管理職判定は `user_master.role === 'manager'` の完全一致で、`sales` / `admin_staff` / `sysadmin` は閲覧不可。管理職用URL生成と通知関数も実装済み。デプロイAは未更新、公開版summaryページ未確認、managerでの閲覧未確認、salesでの拒否未確認、`notifyManagerSummary` 未実行、時刻トリガー未設定、OUT-03完了確定前、OUT-04未着手。** `src/01_auth.gs`、`src/06_output.gs`、`src/summary.html` を経由した実装内容であり、正式なTOP画面URLは変更していない。
