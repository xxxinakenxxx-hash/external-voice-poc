...
最終更新：2026-07-14（EX-05R「トークン検証の確認手段の整備」を実装中。`external/voice-poc/index.html` を検証専用ページとして改修し、GAS送信先URL、トークン、inputText の入力欄、`#token=` 自動反映、HTTPステータスと応答本文の表示、`text/plain;charset=UTF-8` の POST を追加した。`src/05_token.gs` に検証専用の短命トークン発行関数 `issueShortLivedTokenForTest_` を追加し、EX-05の検証ロジック本体は変更していない。`external/voice-poc/README.md` を検証専用ページの説明に更新した。`external/input-page/` は変更していない。`clasp push --force`、Git操作、GitHub Pages反映確認はこれから。EX-05は継続中で、正しいトークン・改ざんトークン・空欄・期限切れの結果はけんたろうさん確認待ち。EX-06へは進まない。

---

## 1. 現在地

- 現在の状態：**EX-05R 実装済み、EX-05継続中、けんたろうさんの実機確認待ち。** `external/voice-poc/index.html` を検証専用ページとして改修し、GAS送信先URL、トークン、inputText の入力欄、`#token=` 自動反映、HTTPステータスと応答本文の表示、`text/plain;charset=UTF-8` の POST を追加した。`src/05_token.gs` に検証専用の短命トークン発行関数 `issueShortLivedTokenForTest_` を追加し、EX-05の検証ロジック本体は変更していない。`external/voice-poc/README.md` を検証専用ページの説明に更新した。`external/input-page/` は変更していない。`clasp push --force`、Git操作、GitHub Pages反映確認はこれから。EX-05は継続中で、正しいトークン・改ざんトークン・空欄・期限切れの結果はけんたろうさん確認待ち。EX-06以降には進まない。
...
- `clasp push --force` の実行結果：これから実行する。
- Git操作結果：これから `external/voice-poc/`、`src/05_token.gs`、`tasks/TASK_CURRENT.md` をコミットし、push する。
- GitHub Pages反映結果：これから確認する。
