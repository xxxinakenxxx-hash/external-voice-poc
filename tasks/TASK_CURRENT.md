...
最終更新：2026-07-16（EX-08検証環境の修正を完了。`external/voice-poc/index.html` に GAS送信先URL欄とトークン欄を画面上部へ戻し、submit の送信本文に latitude／longitude／storeName を含めるようにした。`src/05_token.gs` に検証専用の `issueShortLivedTokenForTest()` を再追加した。`external/voice-poc/README.md` と `tasks/TASK_CURRENT.md` を更新した。静的チェック、`clasp push --force`、Git commit/push、GitHub Pages反映を確認済み。EX-08実機確認待ち、EX-09未着手、完了判定未施行。）

---

## 1. 現在地

- 現在の状態：**EX-08検証環境の修正完了、静的チェック・`clasp push --force`・Git commit/push完了、GitHub Pages反映確認済み、EX-08実機確認待ち、EX-09未着手。** `doPost(e)` の現行実体は `src/05_token.gs` のままで、`doPostMemo(e)` は存在しないため、その前提に合わせて action 振り分けを入れた。GPS取得方式の案イや EX-09 の分離方針はそのまま維持し、EX-08では店舗検索の通信疎通だけを実装した。公開ページには action 選択欄が反映された。
- 次にやること：**EX-08実機確認待ち**
...
- GitHub Pages反映結果：`GAS接続設定` とデフォルトの送信先URLが反映確認できた。デプロイB更新は行わない。
