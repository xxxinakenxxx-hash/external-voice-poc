...
最終更新：2026-07-14（EX-06「外部ページとGASの本接続」を実装・静的チェック・`clasp push --force` まで完了。`external/input-page/index.html` を更新し、URLフラグメントの token を sessionStorage に保存してフラグメント削除、token なし時の TOP 画面案内と送信ボタン無効化、`unsent_text` の保持、送信中の二重送信防止、成功時の保存結果表示、失敗時の入力維持を入れた。`src/04_submit.gs` は Session 依存をやめて payload.userKey を使うようにし、`lookupBranch(payload.userKey)` に接続した。`src/05_token.gs` は token 検証後に `submitMemo({ userKey, inputText })` を呼ぶようにし、`issueShortLivedTokenForTest` を削除した。`external/input-page/README.md` を本接続版に更新した。`clasp push --force` は成功したが、Git操作とデプロイ更新は未実施。EX-06完了確定前、次工程未着手、ユーザーによるデプロイ更新・GitHub Pages反映・実機確認待ち。

---

## 1. 現在地

- 現在の状態：**EX-06 外部入力ページの送信先を正しい送信API用デプロイBへ修正済み、GitHub Pages反映結果確認済み、INPUT_PAGE_URL のスクリプトプロパティ変更はユーザー作業として残存、EX-06実機確認前、次工程未着手。** `external/input-page/index.html` の `GAS_SUBMIT_URL` を送信API用デプロイBに更新し、`external/input-page/README.md` にも現在の送信先を反映した。`src/01_auth.gs` ではなく `src/05_token.gs` の `getInputPageUrl_()` が `INPUT_PAGE_URL` スクリプトプロパティを参照しており、TOP遷移先はユーザー側で `https://xxxinakenxxx-hash.github.io/external-voice-poc/external/input-page/` に設定する必要がある。GitHub Pages 反映結果は確認済みだが、`INPUT_PAGE_URL` の変更は Codex からは行っていない。
- 次にやること：**EX-06実機確認前**
